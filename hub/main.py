...
async def save_processed_agent_data(processed_agent_data: ProcessedAgentData):
    print()
    logging.info("Inside save_processed_agent_data")
    json_procesesd_agent_data = processed_agent_data.json()
    logging.info(f"Got processed agent data: {json_procesesd_agent_data}")
    redis_client.lpush("processed_agent_data", json_procesesd_agent_data)
    logging.info("Pushed data to Redis queue")
    ...
    if queue_len >= BATCH_SIZE:
        ...
        logging.info("Saved batch to store")
    logging.info("Reached return")
    print()
    return {"status": "ok"}
